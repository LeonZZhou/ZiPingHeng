...
double mySetAngle,lastAngle, Input,Output,outMin, outMax,ITerm,cmContralVarial;

char hc06Val = 's'; int Speed_need = 0; int Turn_need = 0;
...
double Kp = 10, Ki = 0.5, Kd = 0.06, Ksp = 2.8, Ksi = 0.11;
...
int PWM_left_least = 87; int PWM_right_least = 88;
int count_right = 0;
int count_left  = 0;
int old_time = 0;
int flag;
...
void setup() {
  Angle = 0;
  mySetAngle = 17;  //小车自平衡角度为17
  SampleTime = 100;  //控制器的采样时间100ms
  Wire.begin();
  Serial.begin(9600);
...
/*
  double gap = abs(mySetAngle - Angle); //distance away from setpoint
  if (gap < 10){
  ...
  { //Speed_need=30;Turn_need=0;positions=80;
   ...
   while(timeChange>=SampleTime){
    ...
    positions = constrain(positions, -2300, 2300);//抗积分饱和
      ...
      double angleError = mySetAngle - Angle;
      ITerm+= (Ki * angleError);
      if(ITerm > outMax) ITerm= outMax;
      else if(ITerm < outMin) ITerm= outMin;
      double dAngle = (Angle - lastAngle);
      Output = Kp*angleError + ITerm + Kd*(Gyro_y+dAngle) + Ksp*speeds_filter + Ksi*positions;
    //Output = Kp * (mySetAngle - Angle) + ITerm - Kd*(Angle - lastAngle) + Ksp * (mySetSpeed - speeds) + Ksi * positions + Ksd * (Speed - lastSpeed);
