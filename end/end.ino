...
void loop(){
  Angle_Calcu();
  Input = Angle;
  PID_Compute();  //PID计算获取 Output; Contains the pid algorithm. it should be called once every loop(). Most of the time it will just return without doing anything. At a frequency specified by SetSampleTime it will calculate a new Output.
...
void setupPID() {
  ...
  PID_SetSampleTime(100);  //控制器的采样时间100ms //The default is 200mS; SampleTime: How often, in milliseconds, the PID will be evaluated. (int>0)
  //myPID.SetOutputLimits(0, 2000);//SetOutputLimits(min, max); double main,max
}

//2
void PID_PID(double* Input, double* Output, double* Setpoint,
